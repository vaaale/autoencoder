...
class DirectoryIterator(Iterator):
    ...
    def __init__(self,
        ...
        if not classes:
            classes = []
            for subdir in sorted(os.listdir(x_directory)):
                if os.path.isdir(os.path.join(x_directory, subdir)):
        ...
        function_partial = partial(
            ...
            follow_links=follow_links)
        self.samples = sum(
            pool.map(function_partial, (os.path.join(x_directory, subdir)
        ...
        i = 0
        for dirpath in (os.path.join(x_directory, subdir) for subdir in classes):
            results.append(
    ...
    def next(self):
        ...
        for i, j in enumerate(index_array):
            ...
            x_img = load_img(
                os.path.join(self.x_directory, fname),
                grayscale=grayscale,
                target_size=self.target_size)
            y_img = load_img(
                os.path.join(self.y_directory, fname),
                grayscale=grayscale,
                target_size=self.target_size)
            x = img_to_array(x_img, data_format=self.data_format)
            y = img_to_array(y_img, data_format=self.data_format)
        ...
        # if self.save_to_dir:
        #   for i in range(current_batch_size):
        #     x_img = array_to_img(batch_x[i], self.data_format, scale=True)
        #     fname = '{prefix}_{index}_{hash}.{format}'.format(
        #         prefix=self.save_prefix,
        #         index=current_index + i,
        #         hash=np.random.randint(1e4),
